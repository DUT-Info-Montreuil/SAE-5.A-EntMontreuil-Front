...
import { TimetableService } from 'src/app/core/services/timetable.service';
...
export class TimetableLayoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.timetableService.getEvents(this.actions)
  ...
  eventTimesChanged({ event, newStart, newEnd, type }: SchedulerEventTimesChangedEvent): void {
    ...
    const ev: CalendarSchedulerEvent | undefined = this.events.find(e => e.id === event.id);
    if (ev) {
      ev.start = newStart;
      ev.end = newEnd;
